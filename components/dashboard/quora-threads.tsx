...
type SortOption = "newest" | "oldest" | "relevance-high" | "relevance-low";
...
export function QuoraThreadsPage({ profile, demoMode, demoThreads }: Props) {
  ...
  useEffect(() => {
    if (!localStorage.getItem("comly_quora_hiw_seen")) {
      setShowHiW(true);
      localStorage.setItem("comly_quora_hiw_seen", "1");
    }
  }, []);
  ...
  async function handleRefresh() {
    ...
    setError(false);
    try {
      const res = await fetch("/api/quora-threads", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ profile }),
      });
      ...
      <div className="bg-white border border-[#e5e5e5] rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.10)] overflow-hidden">
        ...
        <div className="px-6 pt-6 pb-5 border-b border-[#f0f0f0]">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: "#b92b271a" }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="https://www.google.com/s2/favicons?domain=quora.com&sz=64" alt="Quora" width={20} height={20} className="w-5 h-5 object-contain" />
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-2">
                <h2 className="text-[18px] font-bold text-[#0a0a0a]">Questions</h2>
          ...
          <div className="flex items-center gap-1">
            {FILTERS.map(({ id, label }) => (
              ...
              >
                {label}
                {id === "new" && newCount > 0 && (
                  <span className="min-w-[16px] h-4 px-1 rounded-full text-[9px] font-bold flex items-center justify-center text-white" style={{ background: "#b92b27" }}>
        ...
        <div className="px-4 pb-5 pt-3 space-y-2">
          {loading ? (
            ...
            <div className="py-12 text-center">
              <p className="text-[13px] font-medium text-[#6b6b6b]">Could not load questions right now.</p>
            ...
            <div className="py-12 text-center">
              <p className="text-[13px] font-medium text-[#6b6b6b]">
                {activeFilter === "saved"   ? "No saved questions yet." :
                 activeFilter === "replied" ? "No answered questions yet." :
                 "No questions found for your niche yet."}
              </p>
              {(activeFilter === "new" || activeFilter === "all") && (
                <button onClick={handleRefresh} className="mt-3 text-[12px] text-[#5B2D91] hover:underline">Refresh to find questions</button>
              )}
            </div>
          ) : (
            displayed.map((t) => {
                ...
                <div key={tid} className="bg-white border border-[#eeeeee] rounded-xl p-4 flex items-start gap-3 hover:border-[#e0e0e0] transition-colors">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-1">
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src="https://www.google.com/s2/favicons?domain=quora.com&sz=32" alt="" width={12} height={12} className="w-3 h-3 opacity-70" />
                      <span className="text-[11px] font-bold" style={{ color: "#b92b27" }}>Quora</span>
                    </div>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {isReplied && (
                        <span className="flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-md bg-emerald-50 text-emerald-600 border border-emerald-200">
                          <CornerUpLeft className="w-2.5 h-2.5" /> Answered
                        </span>
                      )}
                    </div>
                  </div>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-[460px] p-6">
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2.5">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="https://www.google.com/s2/favicons?domain=quora.com&sz=64" alt="" width={20} height={20} className="w-5 h-5" />
                <h3 className="text-[15px] font-bold text-[#0a0a0a]">How Quora Questions works</h3>
              </div>
          ...
          <div className="w-[440px] bg-white h-full overflow-y-auto flex flex-col shadow-2xl">
            <div className="px-5 py-4 border-b border-[#f0f0f0]">
              ...
              <a href={activeThread.url} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-[10px] text-[#aaaaaa] hover:text-[#5B2D91] transition-colors">
                <ExternalLink className="w-3 h-3" /> View on Quora
              </a>
            </div>
