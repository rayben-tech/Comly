...
export const OPENAI_INDICES = [0, 2, 3, 5, 6, 7, 9, 11, 12, 13];
export const GEMINI_INDICES  = [1, 4, 8, 10, 14];
...
export function generateAuditPrompts(profile: BrandProfile): string[] {
  ...
  return [
    ...
    /* 09 ChatGPT    Discovery    */ `My team needs to ${useCase0} more efficiently. What software should we use?`,
    /* 10 Gemini     Open Ended   */ `What ${category} tools would you recommend for ${target_users}? Include well-known and lesser-known options.`,
    /* 11 Perplexity Discovery    */ `I'm a ${target_users} looking to improve ${useCase0}. What's the best tool for my situation?`,
    /* 12 Claude     Discovery    */ `What's the fastest way for a ${target_users} to get started with ${useCase1}?`,
    /* 13 ChatGPT    Discovery    */ `What tools do ${target_users} commonly use to handle ${useCase0} as they grow?`,
    /* 14 Gemini     Open Ended   */ `If you were a ${target_users} starting fresh today, what ${category} tool would you choose and why?`,
  ];
}
